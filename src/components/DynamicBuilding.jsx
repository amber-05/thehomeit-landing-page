...
const DynamicBuilding = () => {
    ...
    return (
        <div ref={containerRef} className="building-canvas" style={{ width: '100%', height: '100%', position: 'absolute', inset: 0, overflow: 'hidden' }}>
            ...
            >
                <defs>
                    <radialGradient id="skyGrad" cx="50%" cy="0%" r="80%">
                        <stop offset="0%" stopColor="#0d0508" />
                ...
                <circle cx="84" cy="10.5" r="3.8" fill="#0d0508" />
            </svg>
            ...
            >
                <ellipse cx="15" cy="22" rx="9" ry="3.5" fill="rgba(180,140,140,0.25)" />
                <ellipse cx="18" cy="20" rx="6" ry="4" fill="rgba(180,140,140,0.15)" />
                <ellipse cx="65" cy="16" rx="12" ry="4" fill="rgba(160,120,120,0.2)" />
                <ellipse cx="69" cy="13.5" rx="7" ry="5" fill="rgba(160,120,120,0.15)" />
            </svg>
            ...
            <svg
                ...
                style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', willChange: 'transform' }}
            >
                {/* Far BG buildings — dark warm */}
                {[
                    [5, 55, 7, 45], [13, 50, 5, 50], [19, 60, 6, 40], [26, 45, 8, 55],
                    [35, 52, 6, 48], [72, 48, 7, 52], [80, 55, 6, 45], [87, 42, 9, 58],
                ].map(([x, y, w, h], i) => (
                    <rect key={i} x={x} y={y} width={w} height={h} fill={`rgba(50,20,25,${0.4 + i * 0.03})`} rx={0.5} />
                ))}
                {/* Horizon glow — brand red */}
                <defs>
                    <linearGradient id="horizonGlow" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="0%" stopColor="rgba(232,64,87,0.12)" />
                        <stop offset="100%" stopColor="rgba(232,64,87,0)" />
                    </linearGradient>
                </defs>
            ...
            >
                ...
                {/* Spire */}
                <rect x="49" y="4" width="2" height="10" rx={1} fill="rgba(232,64,87,0.9)" style={{ filter: 'drop-shadow(0 0 4px rgba(232,64,87,1))' }} />
                {/* Spire tip blink */}
                <circle cx="50" cy="4" r="1.2" fill="#ff8095" className="win-blink-0" style={{ filter: 'drop-shadow(0 0 6px #ff8095)' }} />

                {/* Horizontal floor separators */}
                {Array.from({ length: 12 }, (_, i) => (
                    <line key={i} x1="36" y1={29 + i * 5.8} x2="64" y2={29 + i * 5.8} stroke="rgba(232,64,87,0.1)" strokeWidth={0.2} />
                ))}
                ...
                {/* Windows — tier 2 */}
                {buildingWindows(40, 22, 4, 1, 3.2, 5, 1.4, 'rgba(255,210,180,0.8)', 'rgba(255,180,150,0.6)')}

                {/* Windows — tier 3 */}
                {buildingWindows(43, 15, 3, 1, 3.2, 5, 1.4, 'rgba(255,210,180,0.8)', 'rgba(255,180,150,0.6)')}

                {/* Antenna lights */}
                <circle cx="50" cy="4" r="0.8" fill="#f87171" className="win-blink-2" style={{ filter: 'drop-shadow(0 0 4px #ef4444)' }} />

                {/* Roof ledge accent — brand red */}
                <rect x="35" y="27.5" width="30" height="1.5" fill="rgba(232,64,87,0.6)" rx={0.5} style={{ filter: 'drop-shadow(0 0 4px rgba(232,64,87,0.9))' }} />

                {/* Ground glow under tower */}
                <defs>
                    <radialGradient id="groundGlow" cx="50%" cy="100%" r="40%">
                        <stop offset="0%" stopColor="rgba(232,64,87,0.2)" />
                ...
                {/* Street-level podium */}
                <rect x="30" y="95" width="40" height="5" fill="rgba(25,10,12,0.8)" rx={0.5} />
                <rect x="28" y="93.5" width="44" height="2.5" fill="rgba(232,64,87,0.25)" rx={0.5} style={{ filter: 'drop-shadow(0 0 3px rgba(232,64,87,0.5))' }} />

                {/* Lobby windows */}
                {[0, 1, 2, 3].map(i => (
                    <rect key={i} x={31.5 + i * 8.5} y={89} width={5.5} height={7} rx={0.5}
                        fill="rgba(232,64,87,0.15)"
                        style={{ filter: 'drop-shadow(0 0 4px rgba(232,64,87,0.4))' }} />
                ))}
            </svg>
            ...
            >
                {/* Left FG building */}
                <rect x="0" y="55" width="20" height="45" fill="rgba(14,8,10,0.95)" rx={0.5} />
                <rect x="1" y="56" width="18" height="43" fill="rgba(22,12,15,0.6)" rx={0.3} />
                {buildingWindows(2, 58, 3, 7, 3.5, 4.5, 1.5, 'rgba(251,191,36,0.75)', 'rgba(251,191,36,0.5)')}

                {/* Left FG second building */}
                <rect x="16" y="65" width="14" height="35" fill="rgba(12,6,8,0.9)" rx={0.5} />
                {buildingWindows(17.5, 67, 2, 5, 3.5, 4.5, 1.5, 'rgba(255,200,140,0.65)', 'rgba(255,170,100,0.5)')}

                {/* Right FG building */}
                <rect x="80" y="58" width="20" height="42" fill="rgba(14,8,10,0.95)" rx={0.5} />
                <rect x="81" y="59" width="18" height="40" fill="rgba(22,12,15,0.6)" rx={0.3} />
                {buildingWindows(82, 61, 3, 7, 3.5, 4.5, 1.5, 'rgba(251,191,36,0.75)', 'rgba(251,191,36,0.5)')}

                {/* Right second FG building */}
                <rect x="70" y="68" width="13" height="32" fill="rgba(12,6,8,0.9)" rx={0.5} />
                {buildingWindows(71.5, 70, 2, 5, 3.5, 4.5, 1.5, 'rgba(255,200,140,0.65)', 'rgba(255,170,100,0.5)')}

                {/* Ground / street */}
                <rect x="0" y="97" width="100" height="3" fill="rgba(8,4,5,0.9)" />
                {/* Street lights */}
                {[10, 30, 68, 88].map((lx, i) => (
                    <g key={i}>
                        <rect x={lx} y={88} width={0.7} height={9} fill="rgba(180,160,160,0.4)" />
            ...
            <div style={{
                position: 'absolute',
                inset: 0,
                background: 'radial-gradient(ellipse 60% 40% at 50% 80%, rgba(232,64,87,0.1) 0%, transparent 70%)',
                pointerEvents: 'none',
            }} />
            <div style={{
                position: 'absolute',
                inset: 0,
                background: 'radial-gradient(ellipse 80% 50% at 50% 100%, rgba(15,8,10,0.8) 0%, transparent 60%)',
                pointerEvents: 'none',
            }} />
        </div>
    );
